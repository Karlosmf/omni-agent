...
class Option
{
    ...
    public static function tryFrom(\ReflectionParameter|\ReflectionProperty $member): ?self
    {
        ...
        if (\is_array($self->suggestedValues) && ! \is_callable($self->suggestedValues) && \count($self->suggestedValues) === 2 && ($instance = $reflection->getSourceThis()) && $instance::class === $self->suggestedValues[0] && \is_callable([$instance, $self->suggestedValues[1]])) {
